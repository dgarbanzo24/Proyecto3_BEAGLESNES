...
#if 1 /* BeagleSNES */
void S9xBlitPixSimpleHalfxHalf (uint8 *srcPtr, int srcRowBytes, uint8 *dstPtr, int dstRowBytes, int width, int height)
{
  for (; height > 0; height-=2)
  {
    ...
    srcPtr += (srcRowBytes << 1);
    dstPtr += dstRowBytes;
  }
}

void S9xBlitPixSimple1xHalf (uint8 *srcPtr, int srcRowBytes, uint8 *dstPtr, int dstRowBytes, int width, int height)
{
	width <<= 1;
...
void S9xBlitPixSimpleHalfx1 (uint8 *srcPtr, int srcRowBytes, uint8 *dstPtr, int dstRowBytes, int width, int height)
{
	for (; height; height--)
	{
		uint16  *dP = (uint16 *) dstPtr, *bP = (uint16 *) srcPtr;
