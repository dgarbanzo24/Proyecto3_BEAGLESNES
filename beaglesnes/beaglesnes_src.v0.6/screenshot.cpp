...
{
#if 1 /* BeagleSNES */
	/* For BeagleSNES, we hijack the screenshot functionality to when
	  doing savestates because we will definitely have a rendered
	  screen for making the snapshot image */
	char  filename[PATH_MAX + 1];
	char  drive[_MAX_DRIVE + 1], dir[_MAX_DIR + 1], def[_MAX_FNAME + 1], ext[_MAX_EXT + 1];

	_splitpath(Memory.ROMFilename, drive, dir, def, ext);
	Settings.Paused = TRUE;
	if (doPauseGui(def, width, height))
	{
		emuDone = 1;
		Settings.Paused = TRUE;
	} else {
