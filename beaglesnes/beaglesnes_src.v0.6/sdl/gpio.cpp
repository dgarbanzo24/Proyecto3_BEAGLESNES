...
void gpioEvents(void)
{
	...
	for (i=0; i < GPIO_MAP_SIZE; i++)
	{
		if (changeGPIOState[i])
		{
			...
			switch (i) 
			{
				...
				case 9:  /* Y button */
					event.key.keysym.sym = SDLK_x; break;
				case 10: /* select button */
					event.key.keysym.sym = SDLK_SPACE; break;
				case 11: /* start button */
					event.key.keysym.sym = SDLK_RETURN; break;
				case 12: /* pause button */
					event.key.keysym.sym = SDLK_r; break;
				default:
					break;
			} /* end switch */
	...
	if (changeGPIOState[GPIO_MAP_SIZE - 1])
	{
		if (currentGPIOState[GPIO_MAP_SIZE - 1])
		{
