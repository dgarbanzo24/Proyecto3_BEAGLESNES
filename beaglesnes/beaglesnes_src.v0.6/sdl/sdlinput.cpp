...
/* BeagleSNES */
static uint32_t currentBESPauseCombo = 0;
...
void S9xInitInputDevices (void)
{
	...
	currentBESPauseCombo = 0; /* BeagleSNES */
...
/***** Joystick starts *****/
		...
		case SDL_JOYBUTTONUP:
			/* Capture the pause button, if there is one */
			if ((BESButtonMap[BESDeviceMap[event.jbutton.which]][10] == event.jbutton.button)
				&& (event.type == SDL_JOYBUTTONDOWN))
			{
				Settings.TakeScreenshot = TRUE;
				break;	
			}
			/* Capture the button activity for the pause combo */
			if (event.type == SDL_JOYBUTTONDOWN)
				currentBESPauseCombo |= (1 << event.jbutton.button);
			else
				currentBESPauseCombo &= ~(1 << event.jbutton.button);
			/* Compare to see if the pause combo is active */
			if ( BESPauseCombo && ((currentBESPauseCombo & BESPauseCombo) == BESPauseCombo))
			{
				Settings.TakeScreenshot = TRUE;
				currentBESPauseCombo = 0;
				/* When coming back from pause, unpress all buttons on the 
					player 1 gamepad */
				for (i=0; i < BUTTON_MAP_SIZE; i++)
				{
					S9xReportButton(0x80000000 |
						(BESDeviceMap[event.jbutton.which] << 24) | i, 0);
				}
				break;
			}

			/* Otherwise, pass the button to the emulator */
