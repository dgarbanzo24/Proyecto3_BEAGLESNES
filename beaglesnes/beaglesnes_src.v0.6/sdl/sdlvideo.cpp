...
const char * S9xParseDisplayConfig (ConfigFile &conf, int pass)
{
	if (pass != 1)
		...
		return ("BeagleSNES");
#endif // AWH
	if (conf.Exists("BeagleSNES::VideoMode"))
	{
		GUI.video_mode = conf.GetUInt("BeagleSNES::VideoMode", VIDEOMODE_BLOCKY);
...
#else
GUI.sdl_screen = SDL_SetVideoMode(720,480,16,0);
#endif /* CAPE_LCD3 */
#else
...
#else
		GUI.blit_screen = (uint8 *) GUI.sdl_screen->pixels + 208 + GUI.sdl_screen->pitch * 14;
#endif /* CAPE_LCD3 */
#else
...
void S9xPutImage (int width, int height)
{
	...
	else
	{
		...
		copyHeight = height;
#if defined(CAPE_LCD3)
		blitFn = S9xBlitPixSimpleHalfxHalf;
#else
		blitFn = S9xBlitPixSimple1x1;
#endif
	}
  ...
  /* Update only the necessary portions of the screen */ 
#if defined(CAPE_LCD3)
  /* 320x240 LCD3 display */
  SDL_UpdateRect(GUI.sdl_screen, 
    32,   /* (320-256)/2 */ 
    1,    /* (240-239)/2 */ 
    256,  /* SNES_WIDTH */
    239); /* SNES_HEIGHT_EXTENDED */
#elif defined(BEAGLEBONE_BLACK)
  /* 720x480 HDMI display */
  SDL_UpdateRect(GUI.sdl_screen, 
    104,  /* (720-512)/2 */ 
    1,    /* (480-478)/2 */
    512,  /* SNES_WIDTH * 2 */
    478); /* SNES_HEIGHT_EXTENDED * 2 */
#else
  /* 640x480 display */
  SDL_UpdateRect(GUI.sdl_screen, 
    64,   /* (640-512)/2 */
...
#else
/*Smooth*/
/*S9xBlitPixSimple2x2*/S9xBlitPixSimple2x1((uint8 *) GFX.Screen, GFX.Pitch, GUI.blit_screen, GUI.blit_screen_pitch, width, height);
